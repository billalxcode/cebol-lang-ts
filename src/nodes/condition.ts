import type { CebolBaseNodeInterface, CebolConditionNodeInterface } from "./types";
...
export class CebolConditionNode implements CebolConditionNodeInterface {
    ...
    public toObject(): object {
        return {
            ...
            falseBranch: this.falseBranch ? this.falseBranch.map((node) => node.toObject()) : null,
        };
    }
    ...
    public toString(): string {
        return `CebolConditionNode(${this.condition.toString()}, [${this.trueBranch
            ...
            .join(", ")}], [${this.falseBranch ? this.falseBranch.map((node) => node.toString()).join(", ") : ""}])`;
    }
}